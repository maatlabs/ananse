//! Errors while generating a [`Trace`](crate::Trace).
...
pub enum TraceError {
    ...
    /// A single instruction performed more accesses than the value bus has slots.
    /// Every instruction in Ananse's integer subset fits, so this signals a lift or
    /// executor defect rather than an input the bus is too narrow for.
    #[error("instruction at step {step} performed {count} accesses, more than the value bus holds")]
    AccessOverflow {
        /// The step (row index) of the offending instruction.
        step: usize,
        /// The number of accesses the instruction performed.
        count: usize,
    },
